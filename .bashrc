...
# Docker startup alias
alias dockerup="sudo /usr/local/bin/start-docker.sh"
. "$HOME/.cargo/env"
...
github_dns_check() {
  if ! ping -c 1 github.com &>/dev/null; then
    ...
    if ! grep -q "github.com" /etc/hosts; then
      ...
      echo "GitHub entry already exists in hosts file."
    fi
  fi
}
...
if [ "$PWD" = "$HOME" ]; then
    # Change to specified projects directory
    cd $HOME/coding/projects/

    # Start ssh-agent and run single-password setup
    eval "$(ssh-agent -s)" && ~/coding/scripts/single-password-setup.sh
    
    # Run project selector
    ~/coding/scripts/project-selector.sh
fi
